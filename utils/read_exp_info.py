...
def read_exp_info(filename):
    ...
    lines.Cond = [df.iloc[i,6:].str.lower().str.strip().dropna().values for i in range(df.shape[0])]
    ...
    exp_columns = ['Author', 'Subjects', 'Space', 'Cond', 'XYZmm', 'UncertainTemplates',
                   'UncertainSubjects', 'Smoothing', 'XYZ', 'Kernels', 'MA', 'NumPeaks']
    ...
    experiments.Kernels = experiments.apply(lambda row: kernel_calc(row.Smoothing, 31), axis=1)
    experiments.MA = experiments.apply(lambda row: kernel_conv(row.XYZ.T, row.Kernels), axis=1)
    experiments.NumPeaks = [experiments.XYZ.iloc[i].shape[1] for i in range(num_exp)]
